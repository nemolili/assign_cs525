...
RC forceFlushPool(BM_BufferPool *const bm)
{
	...
	int numFlames = bm->numPages;
	//int recordlen = 12;
	printf("writeBCK-------:%d",numFlames);
	int i;
	for (i=0;i<numFlames; i++)
	{
		PageFlame *pf = &(FH->totalFlames[i]);
		memcpy(ph,pf->data,PAGE_SIZE);
		//strcpy(ph,pf->data);
...
//					strcpy(pf->data,page->data);
					memcpy(pf->data,page->data,PAGE_SIZE);
				}
				return RC_OK;
			}
		}
	...
	switch(bm->strategy)
	{
		case RS_FIFO:
			...
			for(i=0; i<bm->numPages;i++)
			{
				...
				{
					memcpy(page->data,pf->data,PAGE_SIZE);
				...
				if((FH->totalFlames[i]).pageNum == NO_PAGE)
				{
					...
					FH->numReadIO++;
					memcpy(page->data,(FH->first)->data,PAGE_SIZE);
